...
function parseScheduleFields(fields) {
  ...
  const scheduledDays = parseScheduledDays(fields.scheduledDays)
...
function parseRepostBody(body) {
  ...
  const scheduledDays = Array.isArray(body?.scheduledDays)
    ? body.scheduledDays.filter((d) => typeof d === 'string' && d.trim())
    : []
