...
import { createTenant, getAdminAuth, getTenant, hasAdminCredential, updateTenant } from '@flowdesk/firebase'
...
function resolveBillingOrigin(c) {
  ...
  throw new Error('URL do app não detectada. Configure CORS_ORIGIN no backend.')
}

async function requireBillingAuth(c) {
  ...
  return { tenantId: auth.decoded.uid, email: auth.decoded.email }
}

async function ensureBillingTenant(tenantId, email) {
  ...
  return createTenant(tenantId, {
    name: resolvedEmail.split('@')[0] ?? 'Usuário',
    email: resolvedEmail,
  })
}

function sendBillingError(c, err) {
  if (err instanceof z.ZodError) {
    return json(c, 400, { error: 'Plano inválido.' })
  }
  ...
  if (/STRIPE_|Preço Stripe|Stripe não/i.test(message)) {
    return json(c, 503, { error: message })
  }
  if (/Origin|CORS_ORIGIN|URL do app/i.test(message)) {
    return json(c, 400, { error: message })
  }
...
function getSubscriptionBillingPeriod(sub) {
...
async function findActiveSubscriptionForCustomer(stripe, customerId, subscriptionId) {
  if (subscriptionId) {
    try {
      const sub = await stripe.subscriptions.retrieve(subscriptionId)
      if (ACTIVE_SUB_STATUSES.includes(sub.status)) return sub
    } catch {
  ...
  return list.data.find((item) => ACTIVE_SUB_STATUSES.includes(item.status)) ?? null
}

async function resolveStripeCustomerId(stripe, tenant) {
  if (tenant.stripeCustomerId) return tenant.stripeCustomerId
  if (!tenant.email) return null

  const customers = await stripe.customers.list({ email: tenant.email, limit: 20 })
  const byMeta = customers.data.find((cust) => cust.metadata?.tenantId === tenant.id)
  if (byMeta) return byMeta.id

  for (const customer of customers.data) {
    const subs = await stripe.subscriptions.list({ customer: customer.id, status: 'all', limit: 5 })
    const active = subs.data.find((s) => ACTIVE_SUB_STATUSES.includes(s.status))
...
async function reconcileTenantBilling(stripe, tenant) {
...
async function resolveSubscription(stripe, tenant) {
...
export async function billingSyncHandler(c) {
  const auth = await requireBillingAuth(c)
  if (auth.error) return auth.error

  try {
    const stripe = getStripe()
    const tenant = await reconcileTenantBilling(
      stripe,
      await ensureBillingTenant(auth.tenantId, auth.email),
    )
    const sub = tenant.stripeCustomerId ? await resolveSubscription(stripe, tenant) : null
    return c.json({
      ok: true,
      plan: tenant.plan,
...
export async function billingCheckoutHandler(c) {
  const auth = await requireBillingAuth(c)
  if (auth.error) return auth.error
  ...
  try {
    ...
    const tenant = await ensureBillingTenant(auth.tenantId, auth.email)
...
export async function billingPortalHandler(c) {
  const auth = await requireBillingAuth(c)
  if (auth.error) return auth.error
  ...
  try {
    ...
    if (!tenant.stripeCustomerId) {
      return json(c, 400, { error: 'Nenhum cliente Stripe vinculado. Escolha um plano primeiro.' })
    }
    const origin = resolveBillingOrigin(c)
